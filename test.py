...
class CardTestCase(TestCase):
    def __init__(self, *args, **kwargs):
        ...
        for _ in range(12):
            local_card = Unit([UnitType(random.choice(unit_types))], random.randint(0, 9), random.randint(1, 10), random.randint(0, 3))
            local_card.card_id = f"f{str(local_card.unit_types[0].value).zfill(3)}"
            local_deck.append(local_card)

            remote_card = Unit([UnitType(random.choice(unit_types))], random.randint(0, 9), random.randint(1, 10), random.randint(0, 3))
